...
export const CertificationCard = ({
  ...
  return (
      <article  className="flex flex-wrap gap-10 mb-10">
        <div
        ...
        >
        <div className="flex flex-col justify-between h-full relative z-2 p-[2.4rem] ">
        <header  className="absolute top-0 left-0 mt-4 ml-6 text-xs text-gray-400">
            {date}
          </header>
          <div>
          <h5 className="text-xl md:text-xl font-bold mb-4 Welcome-text">{name}</h5>
          <p className="text-sm md:text-base Welcome-text text-white mb-6">{description}</p>
          </div>

          <footer  className="flex justify-end items-center mt-4">
            <a href={CertificationLink} className=" z-40 font-code text-xs font-bold text-n-1 uppercase tracking-wider Welcome-text">
              See Certification
            </a>
